...
local function require_vault_file(action)
  ...
  notify("'" .. action .. "' requires a vault *.md file", vim.log.levels.WARN)
  return false
end
...
local function wire()
  ...
  vim.fn.mkdir(cfg.vault .. "/" .. cfg.images_dir,  "p")
  ...
  if km.insert_image then
    vim.keymap.set("n", km.insert_image,
      function()
        if not require_vault_file("insert image") then return end
        ...
        images.clear_all()
        local all     = images.list()
        if #all == 0 then
          notify("No images found in " .. require("notes.config").get().images_dir .. "/",
            vim.log.levels.WARN)
          return
        end
        require("notes.pickers").pick(all, { prompt = "Insert image" }, function(choice)
...
function M.setup(opts)
  ...
  vim.api.nvim_create_user_command("NotesInit", function()
    ...
    local cwd     = vim.fn.getcwd()
    local existing = files.find_vault_root(cwd)
    if existing then
      notify("Already inside vault root at '" .. existing .. "'", vim.log.levels.WARN)
      return
    end
    files.init_vault(cwd)
  ...
  -- Global keymaps — available before any vault exists
  local km = config.get().keymaps
  if km.init then
    vim.keymap.set("n", km.init, "<cmd>NotesInit<cr>",
      { silent = true, desc = "Notes: initialize vault in cwd" })
  end
  if km.timestamp then
    vim.keymap.set("n", km.timestamp, function()
      local stamp = os.date("%d-%b-%y %H:%M")
      local row, col = unpack(vim.api.nvim_win_get_cursor(0))
      local line = vim.api.nvim_get_current_line()
      vim.api.nvim_set_current_line(line:sub(1, col) .. stamp .. line:sub(col + 1))
      vim.api.nvim_win_set_cursor(0, { row, col + #stamp })
    end, { silent = true, desc = "Notes: insert timestamp" })
  end
end
